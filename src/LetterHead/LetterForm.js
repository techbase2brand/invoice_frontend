...
const LetterForm = () => {
  ...
  const fetchCreditDetail = async (id) => {
    ...
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_BASE_URL}/api/letter-get/${id}`,
        { headers }
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (id) {
        await axios.put(
          `${process.env.REACT_APP_API_BASE_URL}/api/letter-update/${id}`,
          formData,
          { headers }
        );
      } else {
        await axios.post(
          `${process.env.REACT_APP_API_BASE_URL}/api/add-letter-data`,
          formData,
          { headers }
        );
      }
