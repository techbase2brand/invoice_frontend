...
const LetterList = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_API_BASE_URL}/api/get-letter`,
          { headers }
        );
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(
        `${process.env.REACT_APP_API_BASE_URL}/api/delete-letter/${id}`,
        { headers }
      );
          ...
          <tbody>
            {currentItems.map((item) => (
              ...
              >
                ...
                <td className="px-6 py-4">{item.refNo}</td>
                <td className="px-6 py-4">{item.letterHeadDate}</td>
                ...
                <td>
                  <Link to={`/letterHead-pdf/${item._id}`}>
                    <button
                      type="button"
