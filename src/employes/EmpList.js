 ...
 useEffect(() => {
    const fetchData = async () => {
      ...
      if (searchQuery && searchQuery.trim() !== "") {
        apiUrl = `${process.env.REACT_APP_API_BASE_URL}/api/search/employee`;
      } else {
        apiUrl = `${process.env.REACT_APP_API_BASE_URL}/api/get-empData`;
      }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(
        `${process.env.REACT_APP_API_BASE_URL}/api/delete-emp/${id}`,
        { headers }
      );
          ...
          <tbody>
            {currentItems?.map((item) => (
              <tr
                ...
                key={item._id}
              >
                <td className="px-6 py-4">{item.empName || "N/A"}</td>
                <td className="px-6 py-4">{item.familyMember || "N/A"}</td>
                ...
                <td className="px-6 py-4">{item.empCode || "N/A"}</td>
