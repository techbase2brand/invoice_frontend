...
const BankList = () => {
  ...
  const handleDelete = async (id) => {
    try {
      ...
      await axios.delete(
        `${process.env.REACT_APP_API_BASE_URL}/api/delete-detail/${id}`,
        { headers }
      );
          ...
          <tbody>
            {currentItems?.map((item) => (
              <tr
                ...
                key={item._id}
              >
                <td className="px-6 py-4">{item.bankName || "N/A"}</td>
                <td className="px-6 py-4">{item.accNo || "N/A"}</td>
                <td className="px-6 py-4">{item.accType || "N/A"}</td>
                <td className="px-6 py-4">{item.BranchName || "N/A"}</td>
                <td className="px-6 py-4">{item.ifscCode || "N/A"}</td>
                <td className="px-6 py-4">{item.swiftCode || "N/A"}</td>
                <td className="px-6 py-4">{item.accName || "N/A"}</td>
                <td className="px-6 py-4">{item.tradeName || "N/A"}</td>
