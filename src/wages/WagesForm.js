...
const ProForm = () => {
  ...
  // // total/
  // const basicCut = parseInt(basic || "0") + parseInt(med || "0") + parseInt(children || "0") +
  //   parseInt(house || "0") + parseInt(conveyance || "0") + parseInt(earning || "0") +
  //   parseInt(arrear || "0") + parseInt(reimbursement || "0");

  // console.log("basicCut", basicCut)
  // // tax
  // const allTax = parseInt(health || "0") + parseInt(proftax || "0") + parseInt(epf || "0") + parseInt(tds || "0")
  ...
  // const TotalNetSalary = (grossSalary / TotalDays) * TotalLeave;
  // console.log("TotalNetSalary", TotalNetSalary)
  // useEffect(() => {
  //   const TotalLeave = parseInt(causelLeave || '0') + parseInt(medicalLeave || '0') + parseInt(absent || '0');
  //   const TotalDays = parseInt(daysMonth || '0');
  ...
  //   const grossSalaryDeduction = basicCut / TotalDays * TotalLeave;

  //   const allTax = parseInt(health || '0') + parseInt(proftax || '0') + parseInt(epf || '0') + parseInt(tds || '0');

  //   const TotalNetSalary = basicCut - allTax - grossSalaryDeduction;
  //   console.log("TotalNetSalary", TotalNetSalary);

  //   // setnetSalary(TotalNetSalary.toFixed(2));
  //   if (basicCut) {
  //     setnetSalary(basicCut)
  //   } else if (basicCut && allTax) {
  //     const deduct = basicCut - allTax
  //     setnetSalary(deduct)
  //   }else if(basicCut&& TotalDays && TotalLeave){
  //     const grossSalaryDeduction = basicCut / TotalDays * TotalLeave;
  //     const deduct = basicCut - allTax
  //     const total = deduct - grossSalaryDeduction
  //     setnetSalary(total)
  //   }
  // }, [grosssalary, basic, med, children, house, conveyance, earning, arrear, reimbursement, health, proftax, epf, tds, causelLeave, medicalLeave, absent, daysMonth]);
  ...
  useEffect(() => {
    const newBasicCut = parseInt(basic || '0') + parseInt(med || '0') + parseInt(children || '0') +
      parseInt(house || '0') + parseInt(conveyance || '0') + parseInt(earning || '0') +
      parseInt(arrear || '0') + parseInt(reimbursement || '0');
  ...
  useEffect(() => {
    const newAllTax = parseInt(health || '0') + parseInt(proftax || '0') + parseInt(epf || '0') + parseInt(tds || '0');
    setAllTax(newAllTax);
    ...
    if (basicCut) {
      if (allTax) {
        ...
        if (grossSalaryDeduction) {
          ...
          setnetSalary(deduct.toFixed(2));
        }
      } else {
        setnetSalary(basicCut.toFixed(2));
      }
    }
          ...
          <div className='flexer'>
                ...
                {empName &&
                  <>
                    ...
                    >
                      ...
                      <select name="department" value={department} className={defaultInputSmStyle} onChange={(event) => setDepartment(event.target.value)} >
            ...
            <div className='adjust-width'>
              ...
              <div className="client-form-wrapper sm:w-1/2" style={{ width: '100%' }}>
                ...
                <div className="text-sm mb-4"
                >
                  <label className="block text-sm font-medium text-gray-700">Prof. Tax</label>
                  <input
                    type='number'
                ...
                >
                  ...
                  <input
                    ...
                    name='TotalLeave'
                    value={parseInt(causelLeave || '0') + parseInt(medicalLeave || '0') + parseInt(absent || '0')}
                    className={defaultInputSmStyle}
                    disabled={true}
                  />
                </div>
