...
useEffect(() => {
  ...
  const handleDelete = async (id) => {
    try {
      ...
      await axios.delete(
        `${process.env.REACT_APP_API_BASE_URL}/api/delete-client/${id}`,
        { headers }
      );
            ...
            {currentItems?.map((item) => (
              ...
              >
                ...
                <td className="px-6 py-4">
                  <button
                    type="button"
                    class="focus:outline-none text-black bg-yellow-400 hover:bg-yellow-500 focus:ring-4 focus:ring-yellow-300 font-medium rounded-lg text-sm px-5  me-2  dark:focus:ring-yellow-900"
