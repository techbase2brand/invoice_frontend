...
const CostDetailPage = () => {
  ...
  const totalCost = data.projects.reduce((sum, p) => sum + parseFloat(p.cost), 0);
  const totalPending = data.projects.reduce((sum, p) => sum + parseFloat(p.totalPending), 0);
  const totalPaid = totalCost - totalPending;
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="space-y-4">
          {data.projects.map((project, idx) => {
            const completion = ((parseFloat(project.cost) - parseFloat(project.totalPending)) / parseFloat(project.cost) * 100).toFixed(0);
            const recentPayment = project.payments[project.payments.length - 1];
              ...
              <div key={idx} className="bg-slate-800/50 border border-slate-700/50 rounded-xl overflow-hidden backdrop-blur-sm hover:border-slate-600/80 transition-all group">
                ...
                <div className="bg-gradient-to-r from-slate-700/50 to-slate-800/50 p-4 border-b border-slate-700/50">
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-xl font-black text-blue-300">{data.currency} {parseFloat(project.cost).toLocaleString()}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-4 grid grid-cols-1 md:grid-cols-3 gap-4 border-b border-slate-700/50">
                  <div>
                    ...
                    <p className="text-lg font-bold text-slate-200">{data.currency} {parseFloat(project.advance).toLocaleString()}</p>
                  </div>
                  <div>
                    <p className="text-xs text-slate-400 uppercase font-bold mb-2">Pending</p>
                    <p className={`text-lg font-bold ${parseFloat(project.totalPending) > 0 ? 'text-amber-300' : 'text-emerald-300'}`}>
                      {data.currency} {parseFloat(project.totalPending).toLocaleString()}
                    </p>
                  </div>
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-bold text-blue-300 min-w-fit">{completion}%</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-4">
                  ...
                  {recentPayment ? (
                    <div className="bg-slate-700/30 rounded-lg p-3 text-sm">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="font-bold text-emerald-300">{data.currency} {parseFloat(recentPayment.paid).toLocaleString()} paid</span>
                      </div>
                      <div className="flex justify-between items-center mt-2 text-xs text-slate-400">
                        <span>Received: {data.currency} {parseFloat(recentPayment.receivedAmount).toLocaleString()}</span>
                        <span>Pending: {data.currency} {parseFloat(recentPayment.pending).toLocaleString()}</span>
                      </div>
                    </div>
                  ) : (
                  ...
                  {project.payments.length > 1 && (
                    <details className="mt-3 cursor-pointer">
                      <summary className="text-xs text-blue-400 font-bold hover:text-blue-300 transition-colors">
                      ...
                      <div className="mt-3 space-y-2 bg-slate-700/20 rounded-lg p-3">
                        {project.payments.map((payment, pIdx) => (
                          <div key={pIdx} className="flex justify-between items-center text-xs text-slate-300 border-b border-slate-700/30 pb-2 last:border-0">
                            <span>{payment.date}</span>
                            <span className="font-semibold text-emerald-300">{data.currency} {parseFloat(payment.paid).toLocaleString()}</span>
                          </div>
                        ))}
                      </div>
                    </details>
